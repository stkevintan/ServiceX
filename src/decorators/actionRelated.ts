...
function addActionName(
  symbols: ActionSymbols,
  constructor: Function,
  actionName: string
) {
  const decoratedActionNames =
    Reflect.getMetadata(symbols.decorator, constructor) || []
  Reflect.defineMetadata(
    symbols.decorator,
    [...decoratedActionNames, actionName],
    constructor
  )
}
...
export function getAllActionNames(instance: { constructor: Function }) {
  return allActionSymbols.reduce<string[]>(
    (result, symbols) => [
      ...result,
      ...getActionNames(symbols, instance.constructor)
    ],
    []
  )
}
